...
public class Reproducer extends GridCommonAbstractTest {
    ...
    public void testReproducer() throws Exception {
        ...
        IgniteWriteAheadLogManager walMgr = ig0.context().cache().context().wal();

        File walDir = U.field(walMgr, "walArchiveDir");
        ...
        FileWALPointer corruptedCp = (FileWALPointer) checkpoints.get(3).get1();

        FileDescriptor cpFile = walFiles.stream().filter(w -> w.idx() == corruptedCp.index()).findFirst().get();

        WalTestUtils.corruptWalSegmentFile(cpFile, corruptedCp);
        ...
        System.out.println("fullRebalanceInvoked = " + fullRebalanceInvoked);
    }
